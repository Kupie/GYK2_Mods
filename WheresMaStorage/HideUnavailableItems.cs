...
namespace WheresMaStorage
{
	...
	// implementation with the same two-predicate shape (confirmed by grep, not
	// individually read) and are not covered here - see TASKS.md.
	...
	internal static class InventoryWidget_Redraw_Patch
	{
		...
		{
			if (!Plugin.HideUnavailableItems.Value || ___data?.CustomItemsAvailableCondition == null)
			{
				return;
			}
